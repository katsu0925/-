...
# 同梱（bundled shipping）の仕組み

複数商品をまとめて発送するグルーピング機能。2026-07-04 に **メイン自動確定＋販売/発送/完了の自動転記＋集約表示** を追加した。

## データ構造（KV 専用）

グルーピング自体はシート・D1 に書かず Workers KV（CACHE）のみ：

| キー | 値 |
|---|---|
| `bundle:<groupId>` | `{id, members:[管理番号...], updatedAt, main?}` |
| `bundle-of:<管理番号>` | groupId（逆引き O(1)） |

`main` はグループ内で **最初に実価格で販売登録した商品**。main の無いグループ＝レガシー（従来表示・転記なし）。main をグループから外すと main は解除され、レガシー表示に戻る。

- KV プリミティブ: `src/utils/bundle-store.js`（bundles.js と write-proxy.js で共有）
- KV 非原子 read-modify-write は既知の許容制約（DO 原子化はコスト過大で不採用。転記は冪等でレースしても収束）

## 自動転記（fan-out）

`src/handlers/write-proxy.js` の `runBundleFanout_`。トリガーは 2 経路：

1. **save 経路**: `/api/save/details` の保存フィールドが 販売日/販売場所/販売価格/発送日付/発送者/完了日 のいずれかと交差したとき（waitUntil で非ブロック）
2. **join 経路**: `/api/bundles/toggle` で同梱追加が成立したとき（main の現値を新メンバーへ転記）

転記ルール（メンバーごと・冪等）：

| 系統 | 条件 | 転記内容 |
|---|---|---|
| 販売 | メンバーの販売日が空 | 販売日/販売場所=main と同値、販売価格 0・送料 0・手数料 0 |
| 販売（再転記） | メンバーの販売価格が 0/空 | 販売日/販売場所のみ追従 |
| 販売（スキップ） | メンバーに手動の実価格あり | 販売系は転記しない |
| 発送 | main の発送日付あり・メンバー空 | 発送日付（＋発送者）を前進転記 |
| 完了 | main の完了日あり・メンバー空 | 完了日を前進転記 |

- ステータスは直接書かず、日付転記 → `DERIVED_STATUS`（前進のみ・降格禁止）で発送済み/売却済みに遷移
- 書込みは D1 楽観更新＋GAS 投入（シート反映）。Push 通知はメインの 1 通のみ（メンバー分は `suppressPush` で抑止）
- 転記の GAS 失敗は `savefail:` KV に記録。**メインの販売日を再保存すれば再転記＝リトライ手段**（専用リトライ機構は無し）
- 販売データの削除・クリアは自動化していない（誤登録の取り消しは手動）

## フロント集約表示（pages/app.js）

- 発送商品タブの 3 トグル（発送待ち/発送済み/完了）すべてで、main ありグループは **メイン 1 枚＋「📦 同梱N点」バッジ** に集約。トグルの件数もカード枚数と整合
- main がリストに居ない場合（3 か月経過・検索絞り込み等）はメンバーを個別表示する安全網あり
- 検索で管理番号を直指定すればメンバーも単独表示される
- 詳細画面（販売タブ）の同梱カードに「★メイン」印
