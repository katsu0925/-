...
割り当て管理番号は区分コードごとに **点数を累計した連番** です。報告済みの点数をあとから変えて再採番させると、**同じ区分の後続バッチの管理番号が丸ごとズレ**、すでに採寸・撮影・出品まで終わった商品の番号が合わなくなります。原則として **元の報告（点数）は直接書き換えません**。

### 推奨手順：不足分を「新規仕入れ」として追加する（後続バッチが作業済みのとき）

例）区分 ZA を 50 点と報告 → `zA1~50` 採番済み。別の ZA 仕入れが `zA51~80` で登録済み。あとで最初の ZA が実は 52 点だったと判明 → 余った 2 点の対応。

1. アプリの **仕入れ管理タブ（管理者のみ表示）→ 新規登録**
2. 入力：区分コード = 元と同じ（例 ZA）／仕入れ日 = **今日（最新日）**／商品点数 = 不足していた点数（例 2）／金額・送料
3. 保存すると `staff_apiCreatePurchase`（`StaffApi.gs:1628`）が走り、`staff_nextKanriNumber_` が **商品管理シートの同区分の最大番号 + 1** から採番します。末尾に `zA81~82` のように付与され、**既存の `zA51~80` はズレません**

:::warn
新規行の **仕入れ日は必ず最新日**（今日など）にしてください。実際の古い仕入れ日を入れると、再採番のソート（仕入れ日順）で後続バッチの前に割り込み、番号がズレる原因になります。
:::

### 原価について

原価は仕入れ管理の **行ごと** に `(金額 + 送料) ÷ 商品点数` で算出されます（`recalcUnitCost_` / `仕入れ数マージ.gs:55`）。

- 新規行の金額・送料を **0** で登録 → 余剰分の原価は ¥0、元の行の原価はやや高め（仕入れ総額は不変・1 点単価が不正確）
- **正確に揃えたい場合**：元の仕入れ行の金額・送料を点数按分し直し（元の総額 × 元点数/実点数）、余り分を新規行に入れる。**金額・送料の編集は管理番号に影響しません**（安全に再計算されます）

### 後続バッチがまだ未着手のとき（全体を振り直してよい場合）

区分内に作業済み商品がなく、番号がズレても影響がない場合は、**スプレッドシートを直接開いて**修正できます。
...
:::warn
この方法は区分全体の管理番号を振り直します。**後続バッチに採寸・撮影・出品済みの商品が 1 つでもあるなら使わないでください。** その場合は上の「新規仕入れとして追加」で対応します。
:::

### 点数を多く報告しすぎたとき

実際より多く報告した場合（例：52 と報告したが実際は 50）は、余った番号が実在しない商品になります。後続バッチが未着手なら上記のスプレッドシート直接修正で振り直し、作業済みなら余った番号を **欠番** として扱います（無理に詰めない）。
