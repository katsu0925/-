 ...
 */
// ※ 以下の価格は 2026-08-22 に配信済みの本文そのまま（当時の価格）。
//    2026-08-29 の値下げ（小6,800 / 中12,800 / 大19,800）以降にこの文面を使い回す場合は
//    価格・割引後価格・期間をすべて書き換えること。過去の配信記録として原文は残してある。
function premium30NewsletterBody_() {
  ...
  return [
    ...
    '出品キット（Webページ）が付属しますので、届いたその日から「貼るだけ」で出品作業を始められます。',
