...
function om_findRequestRow_(orderSs, receiptNo) {
  ...
  for (var s = 0; s < sheets.length; s++) {
    ...
    var receiptIdx = rIdx.hasOwnProperty('受付番号') ? rIdx['受付番号'] : REQUEST_SHEET_COLS.RECEIPT_NO - 1;
    var values = sh.getRange(2, 1, lastRow - 1, lastCol).getValues();
    for (var i = 0; i < values.length; i++) {
      if (String(values[i][receiptIdx] || '').trim() === target) {
...
/**
 * om_findRequestRow_ の結果から列番号（1始まり）を引く。
 * アーカイブは作られた時期によって列の並びがずれている可能性があるので、
 * 見出し名を優先し、見つからなければ依頼管理の固定列に落とす。
 */
function om_requestColOf_(located, headerNames, fallbackCol) {
  for (var i = 0; i < headerNames.length; i++) {
    if (located.rIdx.hasOwnProperty(headerNames[i])) return located.rIdx[headerNames[i]] + 1;
  }
  return fallbackCol;
}
...
function buildProductRowsForReceipt_(receiptNo) {
  ...
  // 注文時価格JSON。アーカイブは発送サイズ列の移設前の並びのことがあるので、
  // 固定列ではなく見出しから引く（直右の「出品キット」列を目印にする）。
  var orderPriceMap = {};
  var priceCol = om_requestColOf_(located, ['商品単価JSON', '商品単価'],
    located.rIdx.hasOwnProperty('出品キット') ? located.rIdx['出品キット'] : REQUEST_SHEET_COLS.ITEM_PRICES);
  var priceJson = String(reqRow[priceCol - 1] || '').trim();
  try {
    // 新形式 {assort:{},detauri:{}} と旧形式 {id:price} の両対応（om_executeFullPipeline_ と同じ読み方）
    var parsedPrices = priceJson ? JSON.parse(priceJson) : null;
    if (parsedPrices && (parsedPrices.assort !== undefined || parsedPrices.detauri !== undefined)) {
      var assortPrices = parsedPrices.assort || {};
      var detauriPrices = parsedPrices.detauri || {};
      for (var apk in assortPrices) orderPriceMap[apk] = assortPrices[apk];
      for (var dpk in detauriPrices) orderPriceMap[dpk] = detauriPrices[dpk];
    } else if (parsedPrices && typeof parsedPrices === 'object') {
      orderPriceMap = parsedPrices;
    }
  } catch (e) {
    orderPriceMap = {};
  }
  if (!Object.keys(orderPriceMap).length) {
    console.warn('注文時価格JSONが読めません（%s %s列目）。現在価格で組み立てます: %s', located.sheetName, priceCol, receiptNo);
  }
...
function reissueKitForReceipt_(receiptNo) {
  ...
  var kitCol = om_requestColOf_(located, ['出品キット'], REQUEST_SHEET_COLS.KIT_URL);
  var currentKitUrl = String(located.row[kitCol - 1] || '').trim();
...
function om_saveKitToWorkers_(receiptNo, customerName, orderDate, totalPrice, productRows, aiResults, reqSheet, reqDataMap, existingToken) {
  ...
  if (located) {
    var kitCol = om_requestColOf_(located, ['出品キット'], REQUEST_SHEET_COLS.KIT_URL);
    var pickCol = om_requestColOf_(located, ['ピッキングリスト', '確認リンク'], REQUEST_SHEET_COLS.CONFIRM_LINK);
    located.sheet.getRange(located.rowNumber, kitCol).setValue(kitUrl);
    // I列は自動展開の「処理済み」判定にも使われている（om_autoExpand の
    // `if (confirmLink) continue;`）。XLSXのURLの代わりにこれを入れることで
    // 判定はそのまま動く。
    // ただし旧注文の作り直しでは、ここに当時のXLSXのURLが残っている。
    // それは発送メールで渡した記録なので上書きしない（空欄かピッキングURLのときだけ書く）。
    var currentPick = String(located.row[pickCol - 1] || '').trim();
    if (!currentPick || currentPick.indexOf('/pick?token=') >= 0) {
      located.sheet.getRange(located.rowNumber, pickCol).setValue(pickUrl);
    }
  } else {
