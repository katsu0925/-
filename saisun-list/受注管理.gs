...
/**
 * 【ワンオフ】受付番号 20260530002719-307 専用。
 * ZB323欠品→zB161 を手動で選択リストに差し替えた箱を「6/1価格改定(+20%)より前の旧価格」で再生成する。
 *
 * 仕様（2026-06-02 ユーザー確定）:
 *  - お客様支払額 = ¥19,800（既存。シートL列は変更しない＝参考ログのみ）
 *  - 箱の中身カタログ価格 = 現在の選択リスト全商品を【旧価格テーブル PRICE_TIER_TABLE_（コード.gs:1154）+ 状態割引】で算出。
 *    合計は「自然値」（¥26,500ちょうどには丸めない）。お得額 = 合計 − 19,800。
 *  - 確認リンク(XLSX, I列) と デキルン(AJ列) の両方を再生成する。
 *  - 旧売却履歴を削除（重複防止）してからフルパイプラインを silent 実行。
 *  - ZB323 の在庫ステータス（売却済み/BO列）は触らない（ユーザー指定）。
 *
 * 注入点: 依頼管理 AI列(=REQUEST_SHEET_COLS.ITEM_PRICES=35, 注文時価格JSON)に旧価格をフラット形式 {管理番号:価格}
 *         で書き込む。確認リンク・デキルン双方が注文時価格を最優先で読むため、この1箇所で両方に反映される。
 *         ※キット単体経路 buildProductRowsForReceipt_(受注管理.gs:849-853) は旧フラット形式しか解釈しないため、
 *           {assort,detauri} 構造ではなくフラット形式で書く必要がある。
 *
 * GASエディタから実行（clasp run はこのプロジェクト未対応）。
 */
function regenOldPriceFor_20260530002719_307() {
  var receiptNo = '20260530002719-307';
  var CUSTOMER_PAID = 19800; // お客様支払額（参考ログ用。シートは変更しない）

  // 旧価格テーブルで段階価格を返す（getActivePriceTierTable_ は日時ゲートで新テーブルV2を返すため使わない）
  function calcOldTier_(n) {
    if (n < 0) return 0;
    var table = PRICE_TIER_TABLE_; // 旧テーブル（コード.gs:1154、2026-05-31まで有効）
  ...
  // ---- 依頼管理から該当行・選択リスト・現AI列を読む ----
  var orderSs = sh_getOrderSs_();
  var reqSh = sh_ensureRequestSheet_(orderSs);
  var lastRow = reqSh.getLastRow();
  if (lastRow < 2) { console.error('依頼管理が空です'); return; }
  var reqValues = reqSh.getRange(1, 1, lastRow, reqSh.getLastColumn()).getValues();
  var rIdx = {};
  reqValues[0].forEach(function(h, i) { rIdx[String(h || '').trim()] = i; });

  var rowNum = -1, reqRow = null;
  for (var i = 1; i < reqValues.length; i++) {
    if (String(reqValues[i][0] || '').trim() === receiptNo) { rowNum = i + 1; reqRow = reqValues[i]; break; }
  }
  if (rowNum < 0) { console.error('受付番号が見つかりません: ' + receiptNo); return; }

  var selectionStr = String(reqRow[rIdx['選択リスト']] || '');
  var idsRaw = selectionStr.split(/[、,，\s]+/).map(function(s) { return s.trim(); }).filter(Boolean);
  // 重複除去（大文字比較・元の表記を保持）
  var seen = {}, ids = [];
  idsRaw.forEach(function(id) { var u = id.toUpperCase(); if (!seen[u]) { seen[u] = true; ids.push(id); } });
  if (ids.length === 0) { console.error('選択リストが空です'); return; }
  console.log('選択リスト(' + ids.length + '点): ' + ids.join('、'));

  // ---- 商品管理を読んで 仕入れ値 / 状態 / 傷汚れ詳細 / 現ステータス / BO列 を取得 ----
  var shiireSs = SpreadsheetApp.openById(OM_SHIIRE_SS_ID);
  var mainSheet = shiireSs.getSheetByName('商品管理');
  var mData = mainSheet.getDataRange().getValues();
  var mHeaders = mData.shift();
  ...
  mHeaders.forEach(function(h, i) { mIdx[String(h || '').trim()] = i; });
  var idColIdx = mIdx['管理番号'];
  var BO_COL_IDX = 66; // BO列 = 67列目（0-based 66, handleMissingProducts と同じ）
  var mDataMap = {};
  for (var mi = 0; mi < mData.length; mi++) {
    var mk = String(mData[mi][idColIdx] || '').trim();
    if (mk) mDataMap[mk] = mData[mi];
  ...
  ids.forEach(function(id) {
    ...
    var cost = toNumber_(row[mIdx['仕入れ値']]) || 0;
    var condition = row[mIdx['状態']] || '目立った傷や汚れなし';
    var damageDetail = String(row[mIdx['傷汚れ詳細']] || '');
    var price = normalizeSellPrice_(calcOldTier_(cost));
    if (condition === '傷や汚れあり' || condition === 'やや傷や汚れあり' || condition === '全体的に状態が悪い') {
      price = Math.round(price * 0.8);
    } else if (condition === '目立った傷や汚れなし' && damageDetail.trim() !== '') {
      price = Math.round(price * 0.9);
    }
    oldPriceMap[id] = price;
    total += price;
    var curStatus = mIdx['ステータス'] !== undefined ? String(row[mIdx['ステータス']] || '') : '';
    var curBo = String(row[BO_COL_IDX] || '');
    console.log('  ' + id + ': 仕入¥' + cost + ' / ' + condition + ' → 旧価格¥' + price +
      '（現ステータス=' + (curStatus || '空') + ' / BO=' + (curBo || '空') + '）');
  });
  if (missing.length > 0) console.warn('商品管理に見つからない管理番号（スキップ）: ' + missing.join('、'));

  console.log('───────────────────────');
  console.log('箱の中身 旧価格合計（自然値）: ¥' + total.toLocaleString('ja-JP'));
  console.log('お客様支払額（参考）         : ¥' + CUSTOMER_PAID.toLocaleString('ja-JP'));
  console.log('お得額（合計 − 支払額）      : ¥' + (total - CUSTOMER_PAID).toLocaleString('ja-JP'));
  console.log('───────────────────────');

  // ---- 現AI列をバックアップログ（復元用） ----
  var oldJson = String(reqRow[REQUEST_SHEET_COLS.ITEM_PRICES - 1] || '');
  console.log('AI列(注文時価格JSON) 変更前バックアップ:\n' + (oldJson || '(空)'));

  // ---- フラット形式で書き込み（確認リンク・デキルン双方が読める形式） ----
  var newJson = JSON.stringify(oldPriceMap);
  reqSh.getRange(rowNum, REQUEST_SHEET_COLS.ITEM_PRICES).setValue(newJson);
  console.log('AI列 書き込み後: ' + newJson);
  SpreadsheetApp.flush();

  // ---- 旧売却履歴を削除（フルパイプラインが追記するため重複防止） ----
  try {
    om_removeSaleLogByReceipt_(shiireSs, receiptNo);
    console.log('旧売却履歴を削除しました: ' + receiptNo);
  } catch (e) {
    console.warn('売却履歴削除でエラー（続行）: ' + (e.message || e));
  }

  // ---- 確認リンク(XLSX) + デキルン を再生成（フルパイプライン silent） ----
  // 売却反映で選択リスト内の商品（zB161含む）が売却済みになる。ZB323は選択リストに無いので触られない。
  om_executeFullPipeline_([receiptNo], '旧価格お得 再生成(307)', { silent: true, orderSsId: orderSs.getId() });

  console.log('✅ 完了: 確認リンク(XLSX)とデキルンを旧価格で再生成しました（箱合計¥' +
    total.toLocaleString('ja-JP') + ' / お得¥' + (total - CUSTOMER_PAID).toLocaleString('ja-JP') + '）');
}
