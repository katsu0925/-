...
| 注文送信 | 見積もりモード（商品代のみ。送料は後から案内） |
| **送料自動計算 → 決済** | **未実装（本仕様の対象）** |

### 現在のフロー
```
商品選択 → お届け先入力 → 見積もり送信（商品代のみ）
...
- 上限超 or 沖縄 → 「送料は別途ご案内します」+見積もりモードボタン
...
| ファイル | 変更内容 | 状態 |
|---|---|---|
| **コード.gs** | `syncFull_`: 商品管理L列→データ1 Y列に発送方法を同期。メニューに「決済リンク送付」追加 | 未実装 |
| **Config.gs** | `readCols: 25`、`SHIPPING_AREAS`、`SHIPPING_RATES` 定義、`calcShippingByAddress_` | 実装済み（送料テーブル・計算関数） |
| **StateStore.gs** | `pr_readProducts_`: shippingMethod をJSONに追加 | 未実装 |
| **index.html** | 都道府県検知、厚み判定、送料計算・表示、フロー分岐UI、sumBox送料行追加 | 未実装 |
| **KOMOJU.gs** | metadata に送料情報追加。管理者用決済リンク送付関数追加 | 未実装 |
| **SubmitFix.gs** | 送料パラメータ受け取り、L列に送料込み金額保存、備考に送料内訳 | 未実装 |
...
- [ ] 離島の扱い
- [ ] 送料の税込/税別表記（現在の送料表の金額は税込として扱う？）
- [ ] 会員割引は送料にも適用するか（通常は商品代のみに適用）
