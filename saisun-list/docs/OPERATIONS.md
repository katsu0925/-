...
2. **数量割引（2026年4月廃止済み）** — 常に0%（comboBulkフラグはスキーマ維持のみ・計算スロットは保持）
3. **会員割引** — 10%OFF（ログイン必須、会員割引ON時）
4. **クーポン** — rate型 / fixed型 / shipping_free型

割引は備考欄（AD列）に記録される（例: `【会員割引10%OFF】`）。

### 7.2 送料計算

- 箱サイズ: ≤10点=小、>10点=大
- 13地域別料金テーブル（`Config.gs` SHIPPING_RATES）
- 送料無料条件: ダイヤモンド会員 / 送料無料クーポン / 1万円以上（※FHP適用時・アソートの価格破壊商品 `SHIPPING_CONSTANTS.ALWAYS_CHARGE_BULK_IDS` は1万円以上送料無料の対象外。該当商品分のみ送料請求）
- 離島は配送対象外（35+地域）
...
| T | 配送業者 | |
| U | 伝票番号 | |
| V | 発送サイズ | クリックポスト/中箱/大箱。発送者が入力（作業報酬の算定に使用） |
| W | 箱数 | 発送者が入力。空欄=1箱として計算 |
...
| AE | 発送通知 | |
| AF | 備考 | 割引・送料・ポイント情報 |
| AG | 作業報酬 | 数式（`buildRewardFormula_` / Constants.gs:213）。V列・W列から自動計算。手で書き換えても自己修復で戻る |
| AH | 更新日時 | Q列(入金確認)を変更すると自動で打ち直される（手編集=onEdit / 自動更新=`touchRequestUpdatedAt_`） |
| AI | チャネル | デタウリ/アソート/まとめ |
| AJ | 追跡URL | S列を発送済みにすると T列・U列から自動生成 |
| AK | 商品単価JSON | 注文時価格の永続化 |
| AL | 出品キットURL | |
| AM | CP発行日時 | クリックポストのラベルCSV発行日時（二重発行防止マーカー） |
