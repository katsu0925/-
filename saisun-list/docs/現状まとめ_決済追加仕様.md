...
※送料は割引対象外
※数量割引（旧 10点5%〜100点20%）は2026年4月に廃止（常に0%）
...
| PayPay | **申請中** | - | - |
| Paidy（あと払い） | **利用可能** | なし | KOMOJUが立替入金 |
| Apple Pay | **申請中** | - | - |
| LINE Pay | サービス終了 | - | - |
| メルペイ | 使用しない | - | - |
...
payment.captured / payment.authorized
  ...
  → authorized + konbini/bank_transfer = '入金待ち'
  → captured（入金完了）= '未対応'

payment.updated
  → handlePaymentUpdated_() → updateOrderPaymentStatus_()
  → コンビニ・銀行振込の入金完了時に「入金待ち」→「未対応」に更新
  → KOMOJUダッシュボードで payment.updated イベントを有効にすること
