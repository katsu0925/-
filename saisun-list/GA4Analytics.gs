/* ============================================================
   ...
   ============================================================
   ■ 初回セットアップ
   1. GASエディタ → サービス(+) → 「Google Analytics Data API」 → 追加
   2. ga4SyncAll() を実行して動作確認
   3. ga4SetupDaily() を実行して毎朝6時の自動更新を設定
   ============================================================ */
...
/** 毎朝6時の自動更新トリガーを設定 */
function ga4SetupDaily() {
  ScriptApp.getProjectTriggers().forEach(function(t) {
    if (t.getHandlerFunction() === 'ga4SyncAll') ScriptApp.deleteTrigger(t);
  });
  ScriptApp.newTrigger('ga4SyncAll').timeBased()
    .everyDays(1).atHour(6).inTimezone('Asia/Tokyo').create();
}
...
function ga4Daily_(ss, s, e) {
  ...
  var n  = rows.length;
  var lR = n + 1;            // last data row
  var tR = n + 2;            // totals row
  ...
  if (n > 1) {
    var ch = sh.newChart().setChartType(Charts.ChartType.AREA)
      .addRange(sh.getRange(1, 1, lR, 1))   // 日付
      .addRange(sh.getRange(1, 2, lR, 1))   // ユーザー
      .addRange(sh.getRange(1, 4, lR, 1))   // セッション
      .addRange(sh.getRange(1, 5, lR, 1))   // PV
...
function ga4Source_(ss, s, e) {
  ...
  if (n > 1) {
    ...
    var ch = sh.newChart().setChartType(Charts.ChartType.PIE)
      .addRange(sh.getRange(2, 1, top + 1, 1))   // ソース名
      .addRange(sh.getRange(2, 5, top + 1, 1))   // セッション数
      .setMergeStrategy(Charts.ChartMergeStrategy.MERGE_COLUMNS)
      .setPosition(2, 8, 0, 0)
...
function ga4Event_(ss, s, e) {
  ...
  if (n > 1) {
    ...
    var ch = sh.newChart().setChartType(Charts.ChartType.BAR)
      .addRange(sh.getRange(2, 1, topN + 1, 1))   // イベント名
      .addRange(sh.getRange(2, 2, topN + 1, 1))   // 回数
      .setMergeStrategy(Charts.ChartMergeStrategy.MERGE_COLUMNS)
      .setPosition(2, 8, 0, 0)
