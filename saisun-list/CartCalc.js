...
var CartCalc = (function() {
  ...
  function bulkDiscountLabel(count) {
    if (count >= 100) return '100点割引20%OFF';
    if (count >= 50)  return '50点割引15%OFF';
    if (count >= 30)  return '30点割引10%OFF';
    if (count >= 10)  return '10点割引5%OFF';
    return '';
  }
  ...
  function calculate(input) {
    ...
    var assortAlwaysChargeQty = input.assortAlwaysChargeQty || 0;  // 価格破壊商品の数量（1万円以上ルール無効化）
    ...
    // -- Step 3: FHP未適用時の割引 --
    if (!fhpApplied) {
      // 3a: 数量割引（デタウリのみ、comboBulk !== false なら適用）
      if (det.count > 0 && (!coupon || coupon.comboBulk !== false)) {
        var bRate = bulkDiscountRate(det.count);
        if (bRate > 0) {
          var bAmt = result.detauri.discounted - Math.round(result.detauri.discounted * (1 - bRate));
          result.detauri.discounted = Math.round(result.detauri.discounted * (1 - bRate));
          if (bAmt > 0) {
            result.detauri.discounts.push({ label: bulkDiscountLabel(det.count), amount: bAmt, style: 'bulk' });
          }
        }
      }
    ...
    if (det.count > 0 && addrPref) {
      ...
      if (area && rates[area]) {
        if (diamondFree) {
          ...
          result.detauri.shipping = { amount: 0, label: 'デタウリ 送料', isFree: true, freeReason: '商品合計1万円以上' };
        } else {
          // 厚み分類→サイズ判定→料金計算
          var t = classifyThickness(det.items);
          if (t.total > 0) {
            var sz = calcShippingSize(t.thick, t.thin);
            if (!sz.size) {
              // 上限超過: 複数口
              var multi = calcMultiShipment(t.thick, t.thin, rates[area]);
              result.detauri.shipping = { amount: multi.amount, label: 'デタウリ 送料（' + escHtml(addrPref) + '・' + multi.sizeLabel + '）', isFree: false, freeReason: '' };
            } else {
              var rIdx = (sz.size === 'small') ? 0 : 1;
              result.detauri.shipping = { amount: rates[area][rIdx], label: 'デタウリ 送料（' + escHtml(addrPref) + '・' + sz.label + '）', isFree: false, freeReason: '' };
            }
          }
        }
      }
    ...
    if (ass.totalQty > 0 && addrPref) {
      ...
      if (aArea && rates[aArea]) {
        if (diamondFree) {
          result.assort.shipping = { amount: 0, label: 'アソート 送料', isFree: true, freeReason: 'ダイヤモンド会員特典' };
        } else if (shippingFreeCoupon) {
          // 送料無料クーポンの除外商品対応
          if (assortExcQty > 0) {
            var excShip = rates[aArea][1] * assortExcQty;
            result.assort.shipping = { amount: excShip, label: 'アソート 送料（' + escHtml(addrPref) + '・大×' + assortExcQty + '、一部商品除外）', isFree: false, freeReason: '' };
          } else {
            result.assort.shipping = { amount: 0, label: 'アソート 送料', isFree: true, freeReason: 'クーポン適用' };
          }
        } else if (thresholdFree) {
          // 1万円以上送料無料 — ただし価格破壊商品は対象外（数量分だけ送料請求）
          if (assortAlwaysChargeQty > 0) {
            ...
            result.assort.shipping = { amount: 0, label: 'アソート 送料', isFree: true, freeReason: '商品合計1万円以上' };
          }
        } else {
          var assortShipAmt = rates[aArea][1] * ass.totalQty;
          result.assort.shipping = { amount: assortShipAmt, label: 'アソート 送料（' + escHtml(addrPref) + '・大×' + ass.totalQty + '）', isFree: false, freeReason: '' };
        }
      }
    }
    ...
    if (pointsUsed > 0 && customer) {
      ...
      result.pointsApplied = Math.min(pointsUsed, maxPts, Math.max(0, sumSubtotals));
    }
