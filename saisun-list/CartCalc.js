...
var CartCalc = (function() {
  ...
  function calculate(input) {
    ...
    // ¥10,000以上で送料無料（FHP・沖縄・半額級クーポン(NLHALF-)は対象外）
    var noThresholdCoupon = !fhpApplied && !!(coupon && coupon.noThresholdFreeShip);
    var thresholdFree = !fhpApplied && !isOkinawa && !noThresholdCoupon && combinedDiscountedProduct >= freeShipThreshold;
    ...
    if (!fhpApplied && !isOkinawa && !noThresholdCoupon && combinedDiscountedProduct > 0 && combinedDiscountedProduct < freeShipThreshold) {
    ...
    } else if (!fhpApplied && !isOkinawa && !noThresholdCoupon && combinedDiscountedProduct >= freeShipThreshold) {
