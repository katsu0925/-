...
function setChatbotApiKey() {
  ...
  if (key === 'sk-xxxxx') {
    console.log('エラー: APIキーを実際の値に置き換えてから実行してください。');
    return;
  }
  PropertiesService.getScriptProperties().setProperty('OPENAI_API_KEY', key);
  console.log('OPENAI_API_KEY を設定しました。セキュリティのため、このコード内のキー文字列を削除してください。');
}
...
function removeChatbotApiKey() {
  ...
  console.log('OPENAI_API_KEY を削除しました。定型応答モードに切り替わります。');
}
...
function chatbot_buildSystemPrompt_() {
  ...
  return [
    ...
    '',
    '【営業日・営業時間】',
    '・ご注文は365日24時間いつでも可能です（ECサイトなので常時営業）',
    '・発送・事務処理：平日 10:00〜18:00',
    '・土日祝日は発送・事務処理はお休みです（メッセージへの返信は可能です）',
    '・営業日＝平日（土日祝を除く）です',
    '',
    '【送料について】',
    '・送料は地域と荷物サイズ（60〜160サイズの5段階）で決まります。点数と商品の厚みから箱サイズを自動計算します',
    '・薄手の商品1点だけのご注文は全国一律280円（クリックポスト・沖縄県も対象）',
    '・関西エリア：750〜1,110円',
    '・関東エリア：780〜1,730円',
    '・北海道：830〜2,720円',
    '・沖縄：2,500〜3,500円',
    '・離島は配送対象外の場合があります',
    '・ダイヤモンド会員は送料無料（沖縄県含む）',
    '・商品合計¥10,000以上で送料無料（※初回50%OFF適用時・沖縄県・アソートの価格破壊商品は対象外で送料別途）',
    '',
    '【新商品の入荷について】',
    ...
    '・シルバー（年間5万円以上）：ポイント3%',
    '・ゴールド（年間20万円以上）：ポイント5%',
    '・ダイヤモンド（年間50万円以上）：ポイント5% + 送料無料（沖縄県含む）',
    '・判定は判定時点から過去12ヶ月のローリング方式（固定更新日なし）',
    '・救済復活：前年(12〜24ヶ月前)ダイヤ/ゴールドの方はレギュラー降格後も直近1ヶ月¥50,000以上購入で即復活',
    '・復活ゴールド特例：年間¥350,000でダイヤ昇格（通常¥500,000）',
    '',
    '【対応ルール】',
    '・日本語で丁寧に回答してください',
    '・回答は簡潔にまとめてください（200文字以内目安）',
    '・分からないことや回答に自信がないことは正直に伝え、「ページ上部の ✉ お問い合わせ からご連絡ください」と案内してください',
    '・回答の最後に「解決しない場合はページ上部の ✉ お問い合わせ からお気軽にご連絡ください」と添えてください',
...
function chatbot_fallbackReply_(message) {
  ...
  var rules = [
    // --- 発送日数 ---
    [['何日', '日数', '届く', '届け', '到着', 'いつ届', 'いつ発送'],
     '入金確認後、3〜5営業日（土日祝休み）以内に発送いたします。\n' +
     '発送完了後、追跡番号をメールでお知らせします。\n\n' +
     '【営業日について】\n' +
     '・発送・事務処理：平日 10:00〜18:00\n' +
     '・土日祝は発送・事務処理はお休みです\n' +
    ...
    // --- 営業時間 ---
    [['営業', '休み', '休日', '祝日', '土日', '対応時間', '何時'],
     'ご注文は365日24時間いつでも可能です。\n\n' +
     '【発送・事務処理】\n' +
     '・平日 10:00〜18:00\n' +
     '・土日祝日は発送・事務処理はお休みです\n' +
     '・メッセージへの返信は土日祝日も可能です\n\n' +
    ...
    [['送料', '配送', '発送'],
     '【送料の目安（税込）】\n' +
     '・薄手の商品1点のみ：全国一律280円（クリックポスト・沖縄県も対象）\n' +
     '・関西：750〜1,110円\n' +
     '・関東：780〜1,730円\n' +
     '・北海道：830〜2,720円\n' +
     '・沖縄：2,500〜3,500円\n\n' +
     '点数と商品の厚みに応じて箱サイズ（60〜160サイズ）を自動計算します。\n' +
     'お届け先の住所入力後に自動計算されます。\n' +
     '※離島は配送対象外の場合があります。\n\n' +
     '【送料無料の条件】\n' +
     '・ダイヤモンド会員（年間50万円以上購入）※沖縄県も対象\n' +
     '・商品合計¥10,000以上\n' +
     '※初回50%OFF適用時・沖縄県・アソートの「価格破壊商品」は¥10,000以上でも対象外です（商品カードに「送料別途」と表示されます）。\n\n' +
     '入金確認後、3〜5営業日（土日祝休み）以内に発送いたします。'],
    ...
    [['注文', '購入', '買い方', '買う', '頼み方', 'カート', '手順', '流れ', 'やり方', '使い方'],
     ...
     '4. 決済方法を選んで注文確定\n\n' +
     '※1点からご注文いただけます。在庫は先着順のためお早めにどうぞ。'],
    ...
    [['会員', 'ランク', 'ポイント', '登録', 'アカウント', 'ログイン', 'サインアップ'],
     '【会員ランク制度】\n' +
     '判定時点から過去12ヶ月の購入額で自動判定（ローリング方式・固定の更新日なし）。\n\n' +
     '・レギュラー：ポイント1%\n' +
     '・シルバー（年間5万円〜）：ポイント3%\n' +
     '・ゴールド（年間20万円〜）：ポイント5%\n' +
     '・ダイヤモンド（年間50万円〜）：ポイント5% + 送料無料（沖縄県含む）\n\n' +
     '【救済復活】\n' +
     '前年(12〜24ヶ月前)にダイヤ/ゴールドだった方はレギュラー降格後も直近1ヶ月¥50,000以上購入で即復活。\n' +
     '救済でゴールド復活された方は年間¥350,000でダイヤ昇格（通常¥500,000）。\n\n' +
     'ポイントは次回のお買い物で1pt=1円としてご利用いただけます。会員登録は無料です。'],
    ...
    [['問い合わせ', '連絡', 'メール', '電話', '相談', 'サポート', '質問'],
     'ページ上部の「✉ お問い合わせ」からご連絡いただけます。\n' +
     'メールでも受け付けております：nkonline1030@gmail.com\n\n' +
     '発送・事務処理：平日 10:00〜18:00（土日祝は発送・事務処理休み）\n' +
    ...
    // --- 最低注文数 ---
    [['最低', '何点', '何枚', '1点', '1枚', '少量'],
     'デタウリは1点からご注文いただけます。薄手の商品1点だけなら送料は全国一律280円（クリックポスト）です。\n' +
     '会員登録で全品10%OFF、商品合計¥10,000以上で送料無料になります（沖縄県は送料無料対象外）。'],
