...
function ga4advice_buildWeeklyPrompt_(daily, yesterdayGA4, weeklyData) {
  var system = [
    ...
    'デタウリは BtoB古着卸売EC。商品は「個品（1点ずつ購入）」と、まとめ買い向けの「アソート（プレミアムアソート 小/中/大の3ロット、おおよそ¥6,800〜¥32,000）」の2系統を既に販売しています。',
    'メインターゲットは副業で古着販売をする個人。1人運営のため実行可能性が重要。',
    '',
    '前提（必ず踏まえること）:',
    '- まとめ買い・セット商品は「アソート」として既に存在する。新しいセット商品／まとめ売りSKUの新規作成は提案しない（既存アソートの露出・導線強化なら可）。',
    '- 個品の数量割引（30点○%等）は2026-04-22に廃止済み。「数量割引を作る／復活させる」は提案しない。',
    '- GA4の流入数・ファネルは生イベント数で、bot・自社アクセスを除外していない。1日あたり数件〜十数件規模の少数データは統計的ノイズが大きいため、少ない数字だけを根拠に「カート→決済で離脱＝UX改善が必要」等と断定しない。サンプルが小さい指標は必ず「参考値」と明記する。',
    '- 注文は週0〜2件の低水準で数ヶ月推移している。単週の増減（前週1件→今週0件など）はノイズの範囲。施策は、単発のサイト内改修より「集客（新規流入を増やす）・需要喚起・既存顧客の再来訪（メルマガ／リピート促進）」を優先的に評価する。',
    '',
    '出力フォーマット（必ずこの形式で）:',
    '### 最優先アクション',
    '[具体的なアクション名]',
    ...
    '- 必ずデータの数値を引用して根拠を示す。ただしサンプルが小さい指標は過大な結論を出さず「参考値」と明記する',
    '- 既存の仕組み（アソート／メルマガ／クーポン）で実現できることは、新規構築ではなくその活用を優先して提案する',
    '- 日本語で回答'
  ].join('\n');
